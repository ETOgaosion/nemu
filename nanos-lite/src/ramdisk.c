...
size_t ramdisk_read(void *buf, size_t offset, size_t len) {
    Assert(offset + len <= RAMDISK_SIZE, "offset: %d, len: %d, RAMDISK_SIZE: %d", offset, len, RAMDISK_SIZE);
...
void init_ramdisk() {
    Log("ramdisk info: start = 0x%lx, end = 0x%lx, size = %d Mbytes", &ramdisk_start, &ramdisk_end, RAMDISK_SIZE / (0x1 << 20));
}

...
/* bus interface */
word_t mmio_read(Decode *s, paddr_t addr, int len) {
  return map_read(s, addr, len, fetch_mmio_map(addr));
}

void mmio_write(Decode *s, paddr_t addr, int len, word_t data) {
  map_write(s, addr, len, data, fetch_mmio_map(addr));
}
